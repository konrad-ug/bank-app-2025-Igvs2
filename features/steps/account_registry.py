...
def field_equals_to(context, pesel, field, value):
    ...
    if field in ["balance"]:
        account_value = float(account.get(field))
        expected_value = float(value)
        assert account_value == expected_value, f"Expected {field}={expected_value}, got {account_value}"
    else:
        assert account.get(field) == value, f"Expected {field}={value}, got {account.get(field)}"
